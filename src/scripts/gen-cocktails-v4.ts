...
const rawlist = JSON.parse(fs.readFileSync('/tmp/missing_batch4.json', 'utf8'));
    ...
    for (let i = 0; i < cocktailsToAdd.length; i += BATCH_SIZE) {
        ...
        try {
            ...
            try {
                ...
                allCocktails = [...allCocktails, ...object.cocktails];
                console.log(`Successfully generated ${batch.length} cocktails on retry.`);
            } catch(e2) {
                 console.error(`Failed batch on retry:`, e2);
            }
        }
    }
    
    fs.writeFileSync('/tmp/generated_cocktails_v4.json', JSON.stringify(allCocktails, null, 4));
    console.log("DONE! Wrote full JSON array to /tmp/generated_cocktails_v4.json");
}
