...
export default function CocktailCard({
    ...
    return (
        <div ref={cardRef} className={`bg-gray-900 border ${isExporting ? 'border-transparent' : 'border-gray-800 hover:border-[var(--primary)] focus-within:border-[var(--primary)]'} rounded-3xl p-6 transition-all duration-300 hover:scale-[1.02] flex flex-col h-full group group/card shadow-2xl overflow-hidden relative`}>
            ...
            {/* Action Bar */}
            <div className={`flex items-center gap-2 mt-auto pt-6 relative z-30 ${isExporting ? 'hidden' : 'flex'}`}>
                <Link
                    href={href}
                    className="flex-1 flex items-center justify-center gap-2 py-3 bg-[var(--primary)] hover:bg-[var(--primary-glow)] text-black rounded-xl font-bold uppercase tracking-wide transition-all shadow-[0_0_15px_rgba(var(--primary-rgb),0.3)] hover:scale-[1.03]"
                >
                    View Recipe
                </Link>
                ...
                <div className="relative group/tooltip flex-1">
                    <Link
                        ...
                        className="w-full flex items-center justify-center gap-2 py-3 bg-[var(--primary)]/10 hover:bg-[var(--primary)]/20 text-[var(--primary)] rounded-xl font-bold uppercase tracking-wide border border-[var(--primary)]/30 transition-colors"
                    >
                        Evolve 🧬
                    </Link>
                    <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-[220px] text-center px-3 py-2 bg-gray-950 border border-gray-800 text-gray-300 text-xs rounded-lg opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-50 shadow-2xl">
                        Create a variation based on your bar and preferences.
                    </span>
                </div>
                ...
                <div className="relative" ref={shareMenuRef}>
                    <button
                        ...
                        className="flex items-center justify-center bg-gray-950 hover:bg-white text-gray-400 hover:text-black border border-gray-800 hover:border-white rounded-xl min-w-[48px] min-h-[48px] text-lg transition-all shadow-[0_0_10px_rgba(0,0,0,0.5)]"
                    >
                        🔗
                    </button>
                    {isShareOpen && (
